...
void AudioEngine::startPlayingRecordedStream() {
    ...
    if (mPlaybackStream) {
        ...
        LOGE(TAG, "startPlayingRecordedStream(): Failed to create recording (%p) stream",
             mRecordingStream);
        closeStream(mPlaybackStream);
    }
...
void AudioEngine::startPlayingFromFile(const char *filePath) {
    ...
    if (mPlaybackStream) {
        ...
        LOGE(TAG, "startPlayingFromFile(): Failed to create recording (%p) stream",
             mRecordingStream);
        closeStream(mPlaybackStream);
    }
...
//        assert(mPlaybackStream->getSampleRate() == mSampleRate);
        ...
        LOGE(TAG,
             "openPlaybackStreamFromRecordedStreamParameters(): Failed to create recording stream. Error: %s",
             oboe::convertToText(result));
    }
...
//        assert(mPlaybackStream->getSampleRate() == mSampleRate);
        ...
        LOGE(TAG,
             "openPlaybackStreamFromFileParameters(): Failed to create recording stream. Error: %s",
             oboe::convertToText(result));
    }
